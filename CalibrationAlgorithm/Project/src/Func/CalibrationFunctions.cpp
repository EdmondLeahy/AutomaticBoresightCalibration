...
bool Read_Lidar_points(const char *filename, PointCloudXYZptr cloud)
{
...
void  Normalization_Condition(MatrixXd &xy_i1, MatrixXd &xy_i2, MatrixXd& H1, MatrixXd& H2) {
	...
	MatrixXd dx1 = MatrixXd::Ones(n_p, 1); //xy_i1.block(0, 0, n_p, 1) - cx1 * (MatrixXd::Ones(n_p, 1));
	MatrixXd dy1 = MatrixXd::Ones(n_p, 1); //xy_i1.block(0, 1, n_p, 1) - cy1 * (MatrixXd::Ones(n_p, 1));
	MatrixXd dx2 = MatrixXd::Ones(n_p, 1); //xy_i2.block(0, 0, n_p, 1) - cx2 * (MatrixXd::Ones(n_p, 1));
	MatrixXd dy2 = MatrixXd::Ones(n_p, 1); //xy_i2.block(0, 1, n_p, 1) - cy2 * (MatrixXd::Ones(n_p, 1));
...
UniquePlanes match_scenes(vector<Scene> scenes)
{
	...
	for (int i=1; i<scenes.size();i++){
		...
		*registration_cloud = *registration_scene.planes[0].points_on_plane;
		...
		if (register_clouds(cloud_rel_orientation, registration_orientation, registration_cloud, base_cloud)){

			// ########### DEBUG ######################
			PointCloudXYZptr merged_cloud(new PointCloudXYZ), temp_print_cloud(new PointCloudXYZ);
			transformPointCloud (*registration_cloud, *temp_print_cloud, registration_orientation);
			*merged_cloud += *base_cloud;
			*merged_cloud += *temp_print_cloud;
			pcl::io::savePCDFileASCII("merged_sc1_sc2.pcd", *merged_cloud);
			pcl::io::savePCDFileASCII("sc1.pcd", *base_cloud);
			pcl::io::savePCDFileASCII("sc2.pcd", *registration_cloud);
			pcl::io::savePCDFileASCII("sc2_rotated.pcd", *temp_print_cloud);
			cout << "REMOVE ME" << endl;
			// ##################################################
			...
			for (int j = 0;j<registration_scene.planes.size(); j++){
				...
				for (int k=0;k<unique.unique_planes.size();k++){
					plane_fit = find_plane_fit(unique.unique_planes[k],temp_rot_plane_cloud);
...
vector<Scene> load_scenes(vector<const char*> pcd_files, MatrixXd Orientation_EOP)
{
...
vector<Scene> LoadDebugData(string basedir)
{
	...
	// Scene 1
	string scene1_dir = basedir + "O1_Planes/";
	string orientation1 = scene1_dir + "Orientation.txt";
	string plane0_0 = (scene1_dir + "Cloud_Plane_0.pcd");
	string plane0_1 = (scene1_dir + "Cloud_Plane_1.pcd");
	string plane0_2 = (scene1_dir + "Cloud_Plane_2.pcd");
	string plane_equations1 = scene1_dir + "PlaneEquations.txt";

	pcd_files1.insert(pcd_files1.end(), {const_cast<char*>(plane0_0.c_str()),const_cast<char*>(plane0_1.c_str()),const_cast<char*>(plane0_2.c_str())});

	temp_scene1.planes = get_debug_planes(const_cast<char*>(plane_equations1.c_str()));
	temp_scene1.scene_orientation = get_debug_orientation(const_cast<char*>(orientation1.c_str()));

	PointCloudXYZptr plane_cloud1_1(new PointCloudXYZ), plane_cloud1_2(new PointCloudXYZ), plane_cloud1_3(new PointCloudXYZ);
...
//	string plane1_1 = (scene2_dir + "Cloud_Plane_1.pcd");
//	string plane1_2 = (scene2_dir + "Cloud_Plane_2.pcd");
//	string plane_equations2 = scene2_dir + "PlaneEquations.txt";
//
//	pcd_files2.insert(pcd_files2.end(), {const_cast<char*>(plane1_0.c_str()),const_cast<char*>(plane1_1.c_str()),const_cast<char*>(plane1_2.c_str())});
//
//	temp_scene2.planes = get_debug_planes(const_cast<char*>(plane_equations2.c_str()));
//	temp_scene2.scene_orientation = get_debug_orientation(const_cast<char*>(orientation2.c_str()));
//
//	PointCloudXYZptr plane_cloud2_1(new PointCloudXYZ), plane_cloud2_2(new PointCloudXYZ), plane_cloud2_3(new PointCloudXYZ);
